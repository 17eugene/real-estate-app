...
const EditListing = () => {
  ...
  useEffect(() => {
    const getCurr = async () => {
      const res = await dispatch(listingOperations.getListing(listingId));
      setUploadedImages(res?.payload?.data?.photos);
    };
  ...
  const onFormSubmit = (data) => {
    ...
    dispatch(listingOperations.updateListing({ id: listingId, ...data }));
  };
    ...
    <>
      ...
      <form className={styles.createFrom} onSubmit={handleSubmit(onFormSubmit)}>
        <div className={styles.left}>
          <div className={styles.inputWrapper}>
            <FormInput
              ...
              name="name"
              {...register("name")}
              defaultValue={currentListing?.name}
            />
            {errors.name && (
        ...
        <div className={styles.right}>
          ...
          <Button text="Confirm" type="submit" loading={loading} />
        </div>
      </form>
    </>
  );
